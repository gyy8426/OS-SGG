...
from lib.pytorch_misc import print_para, load_reslayer4
import logging
import random
conf = ModelConfig()
if conf.model == 'motifnet':
    from lib.models.rel_model import RelModel
elif conf.model == 'gkg':
    from lib.models.rel_model_mspk_normal import RelModel
...
def train_batch(b, verbose=False):
    ...
    losses = {}
    if result.rm_obj_dists.requires_grad:
    ...
    if hasattr(result, 'rel_gcn_dists'):
        losses['rel_gcn_loss'] = F.cross_entropy(result.rel_gcn_dists, result.rel_labels[:, -1])
    if hasattr(result, 'rel_dists_graph'):
        rel_labels_graph = (result.rel_labels[:, -1].data > 0).type(torch.cuda.LongTensor)
        rel_labels_graph = Variable(rel_labels_graph)
        losses['rel_loss_graph'] = F.cross_entropy(result.rel_dists_graph,
                                                   rel_labels_graph)
...
def val_epoch():
    ...
    for val_b, batch in enumerate(val_loader):
        num_obj_cls_correct_i, num_obj_sample_i = \
        val_batch(conf.num_gpus * val_b, batch, evaluator)
        num_obj_cls_correct = num_obj_cls_correct + num_obj_cls_correct_i
        num_obj_sample = num_obj_sample + num_obj_sample_i
    evaluator[conf.mode].print_stats()
    print('obj acc: ', (1.0 * num_obj_cls_correct) / (1.0 * num_obj_sample + 1e-8))
    torch.cuda.empty_cache()
    return np.mean(evaluator[conf.mode].result_dict[conf.mode + '_recall'][20]), \
           ...
           (1.0 * num_obj_cls_correct) / (1.0 * num_obj_sample + 1e-8)
...
for epoch in range(start_epoch + 1, start_epoch + 1 + conf.num_epochs):
    ...
    R_20, R_50, R_100, obj_acc = val_epoch()
