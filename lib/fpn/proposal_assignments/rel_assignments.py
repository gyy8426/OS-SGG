...
def rel_assignments(im_inds, rpn_rois, roi_gtlabels, gt_boxes, gt_classes, gt_rels, image_offset,
    ...
    for im_ind in range(num_im):
        ...
        if fg_rels.size == 0 and bg_rels.size == 0:
            ...
            fg_rels = np.array([[0, 1, 0]], dtype=np.int64)  # 31 is on  29 is near
        ...
        all_rels_i = np.zeros((0, 3), dtype=np.int64)
        if time_fg == 0:
            all_rels_i = bg_rels
        if time_bg == 0:
            all_rels_i = fg_rels
        ...
        if all_rels_i.shape[0] == 0:
            all_rels_i = np.array([[0, 1, 0]], dtype=np.int64)  # 31 is on  # this case only for num_fg > 0
