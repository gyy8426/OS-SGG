...
class LinearizedContext(nn.Module):
    ...
    def __init__(self, classes, rel_classes, mode='sgdet',
        ...
        if self.nl_gcn_concept > 0:
            ...
            conceptvg_dict = json.load(
                open('./OS-SGG_file/ConceptNet/conceptvg_dict.json',
                     'r'))
    ...
    def get_concept_graph(self, obj_labels):
        ...
        # if 0 in obj_labels_unique:
        #     obj_labels_unique.remove(0)
        sub_graph_concpt = []
        ...
        if len(obj_labels_unique) <= 1:
            ...
            source_id = list(obj_labels_unique)[0]
            if source_id == 0:
                unique_concept_id = self.concept2id['background']
            else:
                unique_concept_id = self.obj2concept_ind[str(source_id)]
            ...
            for i in obj_labels_unique:
                for j in obj_labels_unique:
                    if i == j:
                        continue
                    if i == 0 or j == 0:
                        source_id = self.concept2id['background']
        ...
        for ind, obj_labels_ind_i in enumerate(obj_labels_np):
            if obj_labels_ind_i == 0:
                cpt_ind = self.concept2id['background']
            else:
