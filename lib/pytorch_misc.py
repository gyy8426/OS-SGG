...
def optimistic_restore_detector(network, state_dict):
    mismatch = False
    own_state = network.state_dict()
    find_keys = []
    for name, param in state_dict.items():
    ...
    missing = set(own_state.keys()) - set(find_keys)
    if len(missing) > 0:
        print("We couldn't find {}".format(','.join(missing)))
        mismatch = True
    return not mismatch
...
def load_reslayer4(detector, ckpt, num_layer):
    for i in range(num_layer):
        ...
        # detector.roi_fmap[i].bn1.running_mean.copy_(ckpt['state_dict']['roi_fmap.' + str(i) + '.bn1.running_mean'])
        # detector.roi_fmap[i].bn1.running_var.copy_(ckpt['state_dict']['roi_fmap.' + str(i) + '.bn1.running_var'])
        # detector.roi_fmap[i].bn2.running_mean.copy_(ckpt['state_dict']['roi_fmap.' + str(i) + '.bn2.running_mean'])
        # detector.roi_fmap[i].bn2.running_var.copy_(ckpt['state_dict']['roi_fmap.' + str(i) + '.bn2.running_var'])
        # detector.roi_fmap[i].bn3.running_mean.copy_(ckpt['state_dict']['roi_fmap.' + str(i) + '.bn3.running_mean'])
        # detector.roi_fmap[i].bn3.running_var.copy_(ckpt['state_dict']['roi_fmap.' + str(i) + '.bn3.running_var'])
        # detector.roi_fmap_obj[i].bn1.running_mean.copy_(ckpt['state_dict']['roi_fmap.' + str(i) + '.bn1.running_mean'])
        # detector.roi_fmap_obj[i].bn1.running_var.copy_(ckpt['state_dict']['roi_fmap.' + str(i) + '.bn1.running_var'])
        # detector.roi_fmap_obj[i].bn2.running_mean.copy_(ckpt['state_dict']['roi_fmap.' + str(i) + '.bn2.running_mean'])
        # detector.roi_fmap_obj[i].bn2.running_var.copy_(ckpt['state_dict']['roi_fmap.' + str(i) + '.bn2.running_var'])
        # detector.roi_fmap_obj[i].bn3.running_mean.copy_(ckpt['state_dict']['roi_fmap.' + str(i) + '.bn3.running_mean'])
        # detector.roi_fmap_obj[i].bn3.running_var.copy_(ckpt['state_dict']['roi_fmap.' + str(i) + '.bn3.running_var'])
        ...
        if i == 0:
            ...
            detector.roi_fmap_obj[i].downsample[0].weight.data.copy_(
                ckpt['state_dict']['roi_fmap.0.downsample.0.weight'])
            detector.roi_fmap_obj[i].downsample[1].weight.data.copy_(
                ckpt['state_dict']['roi_fmap.0.downsample.1.weight'])
            detector.roi_fmap_obj[i].downsample[1].bias.data.copy_(ckpt['state_dict']['roi_fmap.0.downsample.1.bias'])

            # detector.roi_fmap[i].downsample[1].running_mean.copy_(
            #     ckpt['state_dict']['roi_fmap.0.downsample.1.running_mean'])
            # detector.roi_fmap[i].downsample[1].running_var.copy_(
            #     ckpt['state_dict']['roi_fmap.0.downsample.1.running_var'])

            # detector.roi_fmap_obj[i].downsample[1].running_mean.copy_(
            #     ckpt['state_dict']['roi_fmap.0.downsample.1.running_mean'])
            # detector.roi_fmap_obj[i].downsample[1].running_var.copy_(
            #     ckpt['state_dict']['roi_fmap.0.downsample.1.running_var'])
    return None
