...
def obj_edge_rel_vectors(names, wv_type='glove.6B', wv_dir=DATA_PATH, wv_dim=300):
    wv_dict, wv_arr, wv_size = load_word_vectors(wv_dir, wv_type, wv_dim)
    ...
    for i, token in enumerate(names):
        ...
        if wv_index is not None:
            ...
            # Try the longest word (hopefully won't be a preposition
            print("{} -> {} ".format(token, token.split(' ')))
            wv_arr_t = []
            for token_i in (token.split(' ')):
                wv_index = wv_dict.get(token_i, None)
                if wv_index is not None:
                    wv_arr_t.append(wv_arr[wv_index])
                else:
                    print("fail on {}".format(token_i))
            if len(wv_arr_t) > 0 :
                v = 0
                for wv_arr_t_i in wv_arr_t:
                    v = v + wv_arr_t_i
                vectors[i] = v / len(wv_arr_t)
            else:
                print("fail on {}".format(token))
    return vectors
