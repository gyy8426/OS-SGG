...
def load_graphs_one_shot(graphs_file, mode='train', num_im=-1, num_val_im=0, filter_empty_rels=True,
    ...
    for i in range(len(image_index)):
        ...
        if mode == 'train':
            ...
            # print('before: ')
            # for rel_i in rels:
            #     print( gt_classes_i[rel_i[0]], '-', gt_classes_i[rel_i[1]], '-', rel_i[2])
            #     print( boxes_i[rel_i[0]], '-', boxes_i[rel_i[1]], '-', rel_i[2])
            for rel_i in rels:
            ...
            relationships.append(rels)
        num_box = num_box + len(boxes[-1])
        num_class = num_class + len(gt_classes[-1])
        num_tri = num_tri + len(relationships[-1])
    print('load graph finished for ', mode)
    print('load graph finished image ', split_mask.sum())
    print('load graph finished num_box: ', num_box)
    print('load graph finished num_class: ', num_class)
    print('load graph finished num_tri ', num_tri)
    print('load graph finished tri_rel_list ', len(tri_rel_list))
